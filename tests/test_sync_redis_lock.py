"""Tests for sync Redis distributed lock (RedisLock) in rqlite client.
...
def _cleanup_test_tables() -> None:
    """Clean up tables created by sync Redis lock tests."""
    try:
...
class TestSyncRedisLockDistributedSerialization:
    ...
    def test_sync_redis_lock_distributed_serialization(self):
        ...
        Uses threads to demonstrate that RedisLock serializes access correctly
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("DROP TABLE IF EXISTS redis_test_balance")
            cursor.execute("""
                CREATE TABLE redis_test_balance (
            ...
            initial = 5000.0
            cursor.execute(
                "INSERT INTO redis_test_balance (account, balance) VALUES (?, ?)",
                ("SENDER", initial),
            )
            conn.commit()
        finally:
            ...
            try:
                ...
                try:
                    for _ in range(iterations_per_thread):
                        with lock:
                            c.execute(
                                "UPDATE redis_test_balance SET balance=balance-? WHERE account=?",
                                (amount, "SENDER"),
                            )
                            conn.commit()
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT balance FROM redis_test_balance WHERE account=?", ("SENDER",))
            row = cursor.fetchone()
    ...
    def test_sync_redis_lock_with_connection(self):
        ...
        try:
            # CREATE
            cursor.execute("DROP TABLE IF EXISTS redis_lock_transfer")
            cursor.execute("""
                CREATE TABLE redis_lock_transfer (
                    id INTEGER PRIMARY KEY,
            ...
            with lock:
                cursor.execute(
                    "INSERT INTO redis_lock_transfer (sender, receiver, amount, status) VALUES (?, ?, ?, ?)",
            ...
            # SELECT with lock
            with lock:
                cursor.execute("SELECT * FROM redis_lock_transfer")
            ...
            with lock:
                cursor.execute(
                    "UPDATE redis_lock_transfer SET amount=? WHERE sender=?",
                    (150.0, "Alice"),
                )
                conn.commit()
            ...
            # Verify update
            with lock:
                cursor.execute("SELECT amount FROM redis_lock_transfer WHERE sender=?", ("Alice",))
                row = cursor.fetchone()
                assert row is not None
                assert row[0] == 150.0
            ...
            # DELETE with lock
            with lock:
                cursor.execute("DELETE FROM redis_lock_transfer WHERE sender=?", ("Alice",))
                conn.commit()

            # Verify delete
            with lock:
                cursor.execute("SELECT COUNT(*) FROM redis_lock_transfer")
...
class TestSyncRedisLockWithOperations:
    ...
    def test_sync_redis_lock_suppresses_warnings(self):
        ...
        try:
            with warnings.catch_warnings(record=True) as w:
                ...
                assert len(transaction_warnings) == 0, "RedisLock should suppress BEGIN warnings"
        finally:
            cursor.close()
            conn.close()
    ...
    def test_sync_redis_lock_with_select(self):
        ...
        try:
            cursor.execute("DROP TABLE IF EXISTS redis_select_test")
            cursor.execute("CREATE TABLE redis_select_test (id INTEGER PRIMARY KEY, value TEXT)")
            cursor.execute("INSERT INTO redis_select_test (value) VALUES (?)", ("test",))
            conn.commit()

            with lock:
                cursor.execute("SELECT * FROM redis_select_test")
                rows = cursor.fetchall()
                assert len(rows) == 1
        finally:
            cursor.close()
    ...
    def test_sync_redis_lock_with_insert_update_delete(self):
        ...
        try:
            cursor.execute("DROP TABLE IF EXISTS redis_crud_ops")
            cursor.execute("CREATE TABLE redis_crud_ops (id INTEGER PRIMARY KEY, val TEXT)")
            conn.commit()

            with lock:
                cursor.execute("INSERT INTO redis_crud_ops (val) VALUES (?)", ("a",))
                conn.commit()

                cursor.execute("UPDATE redis_crud_ops SET val=? WHERE id=?", ("b", 1))
                conn.commit()

                cursor.execute("SELECT val FROM redis_crud_ops")
                val_row = cursor.fetchone()
                assert val_row is not None
                assert val_row[0] == "b"

                cursor.execute("DELETE FROM redis_crud_ops")
                conn.commit()

                cursor.execute("SELECT COUNT(*) FROM redis_crud_ops")
                count_row = cursor.fetchone()
                assert count_row is not None
                assert count_row[0] == 0
        finally:
...
class TestSyncRedisLockDeadlockPrevention:
    ...
    With RedisLock, all concurrent operations serialize through the lock,
    ...
    def test_sync_redis_lock_no_lock_lost_updates(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("DROP TABLE IF EXISTS redis_lock_test_accounts")
            cursor.execute("""
                CREATE TABLE redis_lock_test_accounts (
                    id INTEGER PRIMARY KEY,
                    account TEXT NOT NULL UNIQUE,
                    balance REAL NOT NULL
                )
            ...
            initial = 10000.0
            cursor.execute(
                "INSERT INTO redis_lock_test_accounts (account, balance) VALUES (?, ?)",
                ("SENDER", initial),
            )
            conn.commit()
        finally:
            ...
            try:
                ...
                try:
                    for _ in range(iterations_per_thread):
                        ...
                        c.execute(
                            "SELECT balance FROM redis_lock_test_accounts WHERE account=?",
                            ("SENDER",),
                        )
                        row = c.fetchone()
                        if row is None:
                        ...
                        c.execute(
                            "UPDATE redis_lock_test_accounts SET balance=? WHERE account=?",
        ...
        # you CANNOT guarantee correctness. With RedisLock (see other tests),
