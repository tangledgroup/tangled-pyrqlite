"""Async cursor class implementing DB-API 2.0 for rqlite.
...
class AsyncCursor:
    """DB-API 2.0 compliant async cursor for rqlite database.

    Attributes:
        connection: The AsyncConnection object that created this cursor.
    ...
    Example:
        ...
        >>> await cursor.execute("SELECT * FROM users WHERE id=?", (42,))
        >>> row = cursor.fetchone()
        >>> await cursor.close()
    """
    ...
    def __init__(
        ...
        Args:
            connection: The parent AsyncConnection object.
    ...
    async def execute(
        ...
        if any(
            cmd in operation_upper
            for cmd in ["BEGIN", "COMMIT", "ROLLBACK", "SAVEPOINT"]
        ):
        ...
        result = await self._execute_single(operation, parameters)
    ...
    def fetchone(self) -> tuple[Any, ...] | None:
        ...
        Note:
            This method is synchronous - it returns buffered data from execute().
            rqlite HTTP API returns all results at once, so no async I/O needed here.
        """
        if self._closed:
            raise InterfaceError("Cursor is closed")
    ...
    async def _execute_single(
        ...
        """Execute a single SQL statement via HTTP.
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    error_data = await response.json() if response.content else {"error": "Unknown error"}
    ...
    def _parse_result(self, result: dict[str, Any]) -> None:
        ...
        if "error" in result:
            ...
            if (
                ...
                or "NOT NULL constraint failed" in error_msg
            ):
    ...
    async def __aenter__(self) -> AsyncCursor:
        """Enter async context manager protocol.
    ...
    async def __aexit__(
        ...
        """Exit async context manager protocol and close cursor.
